...
- `example_workflows/备用初始化导演台工作流.json`：导演台初始化工作流（模型加载器 ref2va UNET / Qwen3-VL CLIP / 视频+音频 VAE + 主节点导演台模式 + 提示词×3，`PrimitiveStringMultiline` 隐藏预连线到「提示词组」）——加载后打开导演台即可直接开工，是当前随插件分发的唯一示例。
