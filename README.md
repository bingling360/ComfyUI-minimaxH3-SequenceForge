...
│
├─ transition.py               # E4 双向过渡重生成的纯逻辑：past|transition|future 三窗划分与双锚
│                              #   keyframe 构造（不依赖 torch，可独立单测）
...
├─ experiments.py              # 实验性功能开关中枢：E1 强化引导桥+段内分片 / E2 全局记忆锚 /
│                              #   E3 运动感知闭环门控 / E4 双向过渡重生成（默认全关）
│
...
- **不触发重做**：改采样参数（steps / cfg / 采样器 / 调度器 / 模型 / fade_ratio / gate / 实验开关）**只影响此后新生成的段**，已生成段沿用存档。
...
1. 段落导轨上勾选要拼接的段（已完成段才可勾，含序章；按链位顺序自动排列，不用手动排序）；
...
- **二采可接独立模型 / LoRA**：主节点有可选的「二采模型」MODEL 槽（`UNETLoader → LoraLoader → 本槽`），专供高清精化二采使用；**不接则沿用一采「模型」**（旧行为不变）。只作用于高清精化二采——一采、E4 缝区过渡重采样、接缝重摇仍用一采模型。可接不同量化 + 不同 LoRA 的独立链（t2v/i2v 用 fl2va、r2v 用 ref2va，别混接）。
