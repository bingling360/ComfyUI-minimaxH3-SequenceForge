...
def build_registry(global_assets=None, project_links=None, legacy_assets=None) -> dict:
    ...
    def _put(ent, origin):
        ...
        if aid not in by_id:
            ...
            full_name = ent.get("file_name") or ent.get("orig_name") or ent.get("original_name")
            if full_name:
                rec["file_name"] = str(full_name).replace("\\", "/").split("/")[-1]
            ...
            for k in ("file", "legacy_file"):
                if ent.get(k) and not rec.get(k):
                    rec[k] = ent[k]
            full_name = ent.get("file_name") or ent.get("orig_name") or ent.get("original_name")
            if full_name and not rec.get("file_name"):
                rec["file_name"] = str(full_name).replace("\\", "/").split("/")[-1]
        if alias:
