...
## 10. 第二轮反馈：按钮摆出来、动作去冗余、进项目库即可用（2026-09-12）

用户原话要点：
> 「暂存的 input、引用到段之类的多余了。进入到项目库，就应该默认可以在各段提示词框出现。
> 这里的按钮根本大部分没必要存在，而且也不应该双击进入界面才有选项，太奇怪了，选项暴露在外面不是更好吗」

**上一轮我理解反了**：把动作收进右键 / 预览器（照抄 Majoor）解决的是"瓦片被按钮挤爆"，
但用户要的是"按钮看得见、并且没有多余的" —— **多余的是那几个动作本身，不是"按钮"这种形式**。

### 改动
| 项 | 前 | 后 |
|---|---|---|
| 动作入口 | 只在右键菜单 / 双击预览器 | **常用按钮直接摆在瓦片上**（首帧图 / 尾帧图 / 调入项目 / →资产 / 二采 / 改名 / 删除） |
| 预览器 | 9 个按钮（与瓦片重复） | 只留「下载 / 标签」，回归"看图"这个本职 |
| 「作为参考素材」（引用到段） | 有 | **删** —— 段卡本来就有引用勾选区，提示词里直接写 `[[别名]]` 也行 |
| 「复制到 input」 | 有 | **删** —— 后端 `lib_stage` 保留，前端不再暴露 |

### 关键修复：进了项目库却不能在提示词里用
- `@` 补全的候选只读 `getDs().ref_assets`（**节点 widget**）
- 而「调入项目」写的是 `manifest.assets`（**项目文件**）→ 两者脱节
- → 新增 `hydratePool()`：在 `refresh()` 里按 `revision` 节流，把 `manifest.assets`
  回填进 widget。这样调入 / 上传 / 改名的素材会**立刻**出现在段卡的引用勾选区
  与提示词框的 `@` 补全里 —— 也就是用户说的"进项目库就应该默认能用"。

验证：`node --check` × 2 通过；回归 **108 passed / 2 failed**（2 个为缺 torch 的环境问题）。
