...
它的段级标签只有 `Duration / Standalone / Prompt` 三个（**全英文**），其中 `Prompt:` 正文才是本工具的产物。

- **单段**：把本工具输出的官方格式文本整段贴进该段的提示词框（后端官方直通，不重包装）。
- **多段**：按段头 + 三标签拼总提示词，每段的 `Prompt:` 正文放对应段的官方格式文本
  （含三字段间空行；I2VA/FL2VA/L2VA 的对齐指令写在正文最前、空一行再接三字段）。
- **资产引用写在正文里**（`@素材名`），与段卡提示词框同一套语义；没有 `Reference:` 标签
  了——旧文本里的 `参考：` / `Reference:` 会被认出、丢弃并提示「已忽略」（出现在 `Prompt:` **之后**时会静默吞进正文，所以别写）。
- 旧「场景/角色/环境音/配乐」四标签同上：解析仍认（仅旧文本可粘回），但一律丢弃并提示。

## 产物语言

**一律英文。** 段头 `[Segment N]`、段级标签 `Duration:` / `Standalone:` / `Prompt:`、结束语 `[END]`、
以及三字段 / 六段式正文全部英文，只有 `<d>[语言] …</d>` 里的对白 / 歌词保原语言。

历史包袱：本项目曾有按语言分的规则文件（`*_zh.txt`）和自研四字段 ref 口径（`<@名字>` / `<#名字:对话>`），
**已全部删除**——中英混排正文会导致严重问题。
