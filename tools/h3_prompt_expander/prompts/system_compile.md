...
**全篇英文**：字段名、section 名、`[Shot N]`、`At MM:SS.mmm`、`(S1)`、
`<d>`/`</d>`、`<Subject N>`/`<Picture N>`/`<Video N>`/`<Audio N>`、`<scenetrans>`/`<cutoff>`、
retention 标记（`fully_preserved` 等）、任务类型前缀（`[reference generation]` 等）、
关键帧对齐指令整句 —— 以及**画面、动作、环境、光照、材质、运镜语义、音色描述**，全部用英文写。

**唯一保留原语言的两处**：对白原文只进 `<d>[Chinese] …</d>` 且逐字不改；
画面内可见文字只进英文双引号且原文不译。

⚠️ 曾经这里写的是"格式骨架英文、主体内容中文"。**已废止** —— 中英混排正文会导致严重问题
（模型会把中文当指令、把英文当内容，产出语言飘忽）。现在一律英文。

**素材引用 `@素材名` 只写在 `subject_definitions` 定义行，不要写进 `detailed_description`**：
user 消息的「随图说明」会列出可用素材名（如 `@回廊场景`、`@少女立绘`）。
后端会把定义行里的 `@名字` 压实成官方标签，正文里一律用 `<Subject N>` / `<Picture N>` /
`<Video N>` / `<Audio N>` 指代 —— **正文里出现 `@名字` 是 bug**（用户报过的"多参时正文里
冒出图片引用"就是这个）。
...
3. **beats 写时间线**：按 beat 顺序写英文动作链，每 beat 一到两句，每句一个动作；单镜只写一个运镜、节拍 ≤3。
...
* `subject_definitions`：每项一行；`<Subject N>` 是抽象出的可复用可见内容；`<Picture N>`/`<Video N>`
  ...
  定义行的写法：`<Subject N> is the content defined by <Picture k>, the reference image "名字.ext".`
  —— **`@素材名` 只允许出现在这一行**。
...
* T2VA 5s：description 约 60–120 英文词。中文企划译过来超了就压缩。
* 约每 3 秒一个切点；每镜至少 3 秒。
